...
namespace ACWatchDog
{
    ...
    static class Injector
    {
        ...
        /// </remarks>
        public static bool RunSuspendedCommaInjectCommaAndResume(string fileName, string arguments, out int pid, string pathOfDllToInject, string dllFunctionToExecute = null)
        {
            pid = -1;
            ...
            if (!kernel32.CreateProcess(null, fileName + " " + arguments, ref pSec, ref tSec, false, CREATE_SUSPENDED, IntPtr.Zero, Path.GetDirectoryName(fileName), ref sInfo, out kernel32.PROCESS_INFORMATION pInfo))
            {
                return false;
            }
            try
        ...
        public static bool Inject(IntPtr processHandle, string pathOfDllToInject, string dllFunctionToExecute = null)
        {
            ...
            uint dwSize = (uint)((pathOfDllToInject.Length + 1) * Marshal.SizeOf(typeof(char)));
            IntPtr allocMemAddress = kernel32.VirtualAllocEx(processHandle, IntPtr.Zero, dwSize, kernel32.AllocationType.Commit | kernel32.AllocationType.Reserve, kernel32.MemoryProtection.ReadWrite);
            ...
            try
            {
                ...
                // Inject the DLL
                IntPtr remoteThreadHandle = kernel32.CreateRemoteThread(processHandle, IntPtr.Zero, 0, loadLibraryAddr, allocMemAddress, 0, IntPtr.Zero);
                ...
                try
                {
                    ...
                    kernel32.GetExitCodeThread(remoteThreadHandle, out uint injectedDllAddress);
        ...
        {
            IntPtr libraryAddress = kernel32.LoadLibrary(pathOfDllToInject);
            ...
            try
            {
                IntPtr functionAddress = kernel32.GetProcAddress(libraryAddress, dllFunctionToExecute);
                if (functionAddress == IntPtr.Zero)
                {
                    return false;
                }
                long functionAddressOffset = functionAddress.ToInt64() - libraryAddress.ToInt64();
                long addressToExecute = injectedDllAddress + functionAddressOffset;
        ...
        {
            IntPtr remoteThreadHandle = kernel32.CreateRemoteThread(processHandle, IntPtr.Zero, 0, addressToExecute, IntPtr.Zero, 0, IntPtr.Zero);
            if (remoteThreadHandle == IntPtr.Zero)
            {
                return false;
            }
            ...
            {
                kernel32.WaitForSingleObject(remoteThreadHandle, kernel32.INFINITE);
                kernel32.GetExitCodeThread(remoteThreadHandle, out uint exitCode);
                if (exitCode != 0)
                {
                    return true;
                }
